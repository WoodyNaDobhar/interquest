...
class FiefAPIController extends AppBaseController
{
	...
	public function store(CreateFiefAPIRequest $request)
	{
		...
		$response = 
			$fiefs->territory->toArray() + 
			[
				'terrain'	=> $fiefs->territory->terrain->toArray()
			]
		;
		if($input['steward_id'] && $input['steward_id'] != ''){
			$response['fief']['steward'] = $fiefs->steward->toArray();
		}
		$response['fief']['fiefdom']['ruler'] = $fiefs->fiefdom->ruler->toArray();

		return $this->sendResponse($response, 'Fief saved successfully');
	}
